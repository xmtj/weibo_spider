                ...
                for page in range(1, page_num):
                    ...
                    if len(info) > 3:
                        for i in range(0, len(info) - 2):
                            ...
                            str_time = info[i].xpath("div/span/text()")
                            if len(str_time) == 2:
                                print('str_time:', str_time[1])
                            ...
                            infos = info[i].xpath("div/a/@href")
                            ##图片url地址【原图共X张】
                            if infos is not None and len(infos) > 0:
                                time.sleep(5)
                                pic_detail = requests.get(infos[0], cookies=self.cookie, headers=self.headers).content
                                selector_pic_detail = etree.HTML(pic_detail)
                                imageList = selector_pic_detail.xpath('/html/body/div')
                                pic_list = []
                                for num in range(1, len(imageList)):
                                    hrefs = imageList[num].xpath('a/@href')
                                    if hrefs is not None and len(hrefs) == 2:
                                        print("hrefs原图:", hrefs[1])
                                        pu = "http://weibo.cn" + hrefs[1]
                                        try:
                                            origin_pic_detail = requests.get(pu,
                                                                             cookies=self.cookie,
                                                                             headers=self.headers).content
                                            selector_origin_pic_detail = etree.HTML(origin_pic_detail)
                                            pic_url_xpath = selector_origin_pic_detail.xpath(
                                                '/html/body/div[3]/a[1]/@href')
                                            if len(pic_url_xpath) > 0:
                                                pic_url = pic_url_xpath[0]
                                            else:
                                                pic_url = pu
                                            ...
                                            print("Exception:", e)
                                        time.sleep(5)
                                self.weibo_pic_urls[i] = pic_list
                                print("self.weibo_pic_urls[i]:", self.weibo_pic_urls[i])
